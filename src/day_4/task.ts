...
const build_starting_obj = (data_length: number): {[key: string]: number} => {
    return Array.from({ length: data_length }, (_, i) => `${i + 1}`).reduce((acc, key) => {
        acc[key] = 1;
        return acc;
...
const handleScore = (result_str: string): number => {
    ...
    const win_set = winning.trim().split(" ").filter((x) => x)
    const result_set = points.trim().split(" ").filter((x) => x && win_set.includes(x))
